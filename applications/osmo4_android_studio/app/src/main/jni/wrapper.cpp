...
int CNativeWrapper::init(JNIEnv * env, void * bitmap, jobject * callback, int width, int height, const char * cfg_dir, const char * modules_dir, const char * cache_dir, const char * font_dir, const char * gui_dir, const char * urlToLoad) {
	...
	bool gyroscope = env->CallBooleanMethod(getPackageManager(env, *callback), hasFeatureID,env->NewStringUTF("android.hardware.sensor.gyroscope"));
	...
	if (gyroscope) gf_cfg_set_key(m_user.config, "General", "hasGyroscope", "yes");
	else gf_cfg_set_key(m_user.config, "General", "hasGyroscope", "no");
